# grc/ — GNU Radio Companion 自定义块

本目录存放供 GNU Radio Companion（GRC）识别的**自定义块定义文件**（`.block.yml`）。
这些块封装了本仓库 Python 信号处理层，使得 `flowgraphs/gnss_tx_main.grc` 可以直接在 GRC 图形界面中使用，而不需要在 GRC 里写任何 Python 代码。
...
grc/
└── blocks/
    ├── gnss_tx_gps_l1_ca_source.block.yml   # PRN1 扩频基带源块
...
| 属性 | 值 |
|------|----|
| GRC 显示名 | GNSS TX PRN1 Source |
| GRC 分类 | [GNSS TX] |
| 输出 | 1 路 complex64 基带流 |

**作用**：调用 `gnss_tx.gr.make_gps_l1_ca_vector_source()`，预生成一个完整导航 bit 周期的 GPS L1 C/A PRN1 扩频基带缓冲区，通过 `blocks.vector_source_c` 循环回放。避免逐 sample 实时计算引起的 USRP underflow。

**参数**：

| 参数 | 说明 | 默认值 |
|------|------|--------|
| PRN ID | 卫星编号（当前仅支持 1） | 1 |
| Samples / Chip | 每个 chip 对应的 sample 数，采样率 = 1.023 MHz × 此值 | 4 |
...
**作用**：封装 `uhd.usrp_sink`，配置采样率、中心频率、增益、天线和带宽。初始化完成后**自动调用 `sink.get_samp_rate()` 读回硬件确认的实际采样率**，并与请求值对比打印到 GRC Console 面板。

**参数**：

| 参数 | 说明 | 默认值 |
|------|------|--------|
| Device Address | UHD 设备地址（如 `"type=b200"` 或 `"serial=8003272"`） | "type=b200" |
...
块初始化时在 GRC Console 面板打印：
```
[INFO] GNU Radio USRP sink requested sample rate : 4092000.000 Sps (4.092000 Msps)
[INFO] GNU Radio USRP sink requested samples/chip: 4.000000
[INFO] GNU Radio USRP sink actual sample rate    : 4092000.000 Sps (4.092000 Msps)
[INFO] GNU Radio USRP sink actual samples/chip   : 4.000000
[INFO] GNU Radio USRP sink sample-rate delta     : +0.000 Sps (+0.000000%)
```

> USRP B210 内部通过整数分频链派生采样率，`get_samp_rate()` 返回的是硬件实际执行值，可能与请求值有微小偏差。若 `sample-rate delta` 不为零，说明硬件做了舍入，实际 samples/chip 偏离理论值。
...
3. 将 `GNSS TX PRN1 Source` → `amplitude_scale (multiply_const)` → `GNSS TX UHD Sink` 连接
...
这两个块与 `scripts/run_tx.py` Python 运行时使用**完全相同的底层函数**：

| GRC 块模板调用 | Python 运行时等价 |
|---------------|------------------|
| `make_gps_l1_ca_vector_source(...)` | `build_replay_samples()` + `blocks.vector_source_c(...)` |
| `uhd.usrp_sink(...)` + `format_uhd_tx_sample_rate_report(...)` | `create_b210_sink(config)` + `format_uhd_tx_sample_rate_report(...)` |
