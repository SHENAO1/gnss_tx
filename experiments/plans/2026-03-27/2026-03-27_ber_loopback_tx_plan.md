...
### 3.3 SNR 估算（30 dB 衰减器场景，验证信号仍足够）

以 TX 输出 +8 dBm、30 dB 衰减器、1 dB 线缆损耗为例：

```
RX 输入功率：       8 − 30 − 1 = −23 dBm   ← 低于绝对最大额定值 25.5 dB，安全
噪声底（4.092 MHz BW，NF = 2 dB）：
  N = −174 + 10·log₁₀(4.092×10⁶) + 2 ≈ −106 dBm
宽带输入 SNR：      −23 − (−106) = +83 dB
GPS C/A 扩频处理增益：+43 dB（10·log₁₀(1023)）
捕获后等效 SNR：    +126 dB  ← 远超要求，BER 理论趋近于 0
```

**结论：30 dB 衰减器下 SNR 依然极高，不影响 BER 测量结果。保持较高 tx_gain 同时加足衰减器，是兼顾信号质量与设备安全的最优策略。**

---

## 四、发端分步执行计划

### Step 0：安全核查（必须最先完成，预计 15 分钟）
...
**完成标志：**
- [ ] 已记录频谱仪实测 P_meas = ________ dBm（tx_gain = ________ dB）
- [ ] 已选定衰减器：________ dB
- [ ] 衰减后 RX 输入估算 = ________ dBm（须 ≤ 0 dBm）
- [ ] 接线顺序：`B210 TX → [衰减器] → [同轴线缆] → B210 RX` 已确认
